...
| D4 | Sync is file-level over user storage (OneDrive first), serialized by ETag/fingerprint CAS. Conflicts keep one version live plus one recoverable conflict copy. The designated upgrade path is a base-copy three-way field patch — never merge algebra or canonical hashes | 007 |
