...
fn resident_protocol_harness_observes_successful_and_stale_publication() {
    ...
    let mut remote = core
        ...
        .vault;
    let remotely_changed_entry = remote
        .root
        .entries
        .iter_mut()
        .find(|entry| entry.id.to_string() == created.id)
        .expect("advance a field on the mutated Entry");
    remotely_changed_entry.username = "remote-reconciled-user".into();
    remotely_changed_entry.modified_at = 1_600_000_000;
    core.add_entry(
        &mut remote,
        &root_id,
        EntryCreate {
    ...
    let reconciled_entry = match harness.command(RuntimeCommand::UpdateEntryFields {
        vault_id: vault_id.clone(),
        ...
        RuntimeResponse::EntryMutationResult(result) => {
            ...
            result.entry.expect("active reconciled Entry")
        }
        other => panic!("expected reconciled Entry mutation, got {other:?}"),
    };
    assert_eq!(reconciled_entry.username, "remote-reconciled-user");
...
fn successful_conflict_split_preserves_local_then_adopts_remote_head() {
    ...
    let conflict_identity = match harness.command(RuntimeCommand::UpdateEntryFields {
        ...
        RuntimeResponse::EntryMutationResult(result) => {
            ...
            assert!(
                result.entry.is_none(),
                "Conflict Split must not return the Local entry preserved in the Conflict Copy"
            );
            result
                .publication
                .conflict_copy_path
...
#[test]
fn conflict_split_from_a_later_pending_commit_survives_restart_without_replaying_stale_local() {
    let core = KeepassCore::new();
    let (base_bytes, local_entry_id) = vault_with_entry_bytes();
    let (remote_bytes, remote_entry_id) = foreign_remote_head_bytes(&base_bytes);
    let mut harness = RuntimeProtocolHarness::resident_with_in_memory_vault(base_bytes);
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id: vault_id.clone(),
            entry_id: local_entry_id.clone(),
            title: "First pending local edit".into(),
            username: "base-user".into(),
            password: "first-local-password".into(),
            url: "https://base.example".into(),
            notes: String::new().into(),
            totp_uri: None,
            custom_fields: vec![],
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id: vault_id.clone(),
            entry_id: local_entry_id.clone(),
            title: "Latest pending local edit".into(),
            username: "base-user".into(),
            password: "latest-local-password".into(),
            url: "https://base.example".into(),
            notes: "must remain in the only Conflict Copy".into(),
            totp_uri: None,
            custom_fields: vec![],
        }),
        RuntimeResponse::EntryMutationResult(result)
    ...
    let active_entries = match harness.command(RuntimeCommand::ListEntries {
        ...
        RuntimeResponse::EntryList(entries) => entries.entries,
        other => panic!("expected active Remote entry list, got {other:?}"),
    };
    assert_eq!(active_entries.len(), 1);
    assert_eq!(active_entries[0].id, remote_entry_id);

    let conflict_items = match harness.command(RuntimeCommand::ListOneDriveChildren {
        parent_item_id: None,
    }) {
        RuntimeResponse::OneDriveItemList(items) => items
            .items
            .into_iter()
    ...
    assert_eq!(
        conflict_items.len(),
        1,
        "restart must not replay the superseded first Local generation"
    );
    let conflict_vault = core
        .load_database(
            &harness.provider_item_bytes(&conflict_items[0].item_id),
            &key(),
        )
        .expect("decode the durable Conflict Copy")
        .vault;
    let local_copy = core
        .project_entry_detail(&conflict_vault, &local_entry_id)
        .expect("latest Local entry in the Conflict Copy");
    assert_eq!(local_copy.title, "Latest pending local edit");
    assert_eq!(local_copy.password, "latest-local-password");
    assert_eq!(local_copy.notes, "must remain in the only Conflict Copy");
}

#[test]
fn completed_conflict_receipt_cannot_replace_a_newer_pending_commit_after_restart() {
    let (base_bytes, local_entry_id) = vault_with_entry_bytes();
    let (remote_bytes, remote_entry_id) = foreign_remote_head_bytes(&base_bytes);
    let mut harness = RuntimeProtocolHarness::resident_with_in_memory_vault(base_bytes);
    harness.command(RuntimeCommand::Handshake {
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id: vault_id.clone(),
            entry_id: local_entry_id,
            title: "Earlier split Local".into(),
            username: "base-user".into(),
            password: "earlier-split-password".into(),
            url: "https://base.example".into(),
            notes: String::new().into(),
            totp_uri: None,
            custom_fields: vec![],
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id: vault_id.clone(),
            entry_id: remote_entry_id.clone(),
            title: "Newest pending Remote-derived Local".into(),
            username: "remote-user".into(),
            password: "newest-pending-password".into(),
            url: "https://remote-only.example".into(),
            notes: "must outrank the old Conflict Receipt".into(),
            totp_uri: None,
            custom_fields: vec![],
        }),
        RuntimeResponse::EntryMutationResult(result)
    ...
    let detail = match harness.command(RuntimeCommand::GetEntryDetail {
        vault_id: restarted_vault_id,
        entry_id: remote_entry_id,
    }) {
        RuntimeResponse::EntryDetail(detail) => detail,
        other => panic!("expected the newer pending Entry after restart, got {other:?}"),
    };
    assert_eq!(detail.title, "Newest pending Remote-derived Local");
    assert_eq!(detail.password, "newest-pending-password");
    ...
    assert_eq!(
        conflict_items.len(),
        1,
        "a historical completed Receipt must not be replayed"
    );
}

#[test]
fn failed_conflict_copy_from_a_later_pending_commit_recovers_the_latest_local_after_restart() {
    let core = KeepassCore::new();
    let (base_bytes, local_entry_id) = vault_with_entry_bytes();
    let (remote_bytes, remote_entry_id) = foreign_remote_head_bytes(&base_bytes);
    let mut harness = RuntimeProtocolHarness::resident_with_in_memory_vault(base_bytes);
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id: vault_id.clone(),
            entry_id: local_entry_id.clone(),
            title: "Latest recoverable local edit".into(),
            username: "base-user".into(),
            password: "latest-recoverable-password".into(),
            url: "https://base.example".into(),
            notes: "durable despite failed Conflict Copy publication".into(),
            totp_uri: None,
            custom_fields: vec![],
        }),
        RuntimeResponse::EntryMutationResult(result)
    ...
    let conflict_vault = core
        ...
        .expect("decode retried durable Conflict Copy")
        .vault;
    let local_copy = core
        .project_entry_detail(&conflict_vault, &local_entry_id)
        .expect("latest Local entry in the retried Conflict Copy");
    assert_eq!(local_copy.title, "Latest recoverable local edit");
    assert_eq!(local_copy.password, "latest-recoverable-password");
    assert_eq!(
        local_copy.notes,
        "durable despite failed Conflict Copy publication"
    );
}

#[test]
fn restart_recovers_a_conflict_receipt_interrupted_before_provider_preservation() {
    let core = KeepassCore::new();
    let (base_bytes, local_entry_id) = vault_with_entry_bytes();
    let (remote_bytes, remote_entry_id) = foreign_remote_head_bytes(&base_bytes);
    let mut harness = RuntimeProtocolHarness::resident_with_in_memory_vault(base_bytes);
    ...
    harness.reject_next_publication_as_stale(remote_bytes);
    harness.interrupt_next_conflict_split_after_receipt_intent();
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id,
            entry_id: local_entry_id.clone(),
            title: "Receipt-only Local".into(),
            username: "base-user".into(),
            password: "receipt-only-password".into(),
            url: "https://base.example".into(),
            notes: "recover after receipt intent".into(),
            totp_uri: None,
            custom_fields: vec![],
        }),
        RuntimeResponse::Error(_)
    ));
    let items_before_restart = match harness.command(RuntimeCommand::ListOneDriveChildren {
        parent_item_id: None,
    }) {
        RuntimeResponse::OneDriveItemList(items) => items.items,
        other => panic!("expected Provider item list, got {other:?}"),
    ...
    let conflict_vault = core
        ...
        .expect("decode recovered Conflict Copy")
        .vault;
    let local_copy = core
        .project_entry_detail(&conflict_vault, &local_entry_id)
        .expect("receipt Local entry in the Conflict Copy");
    assert_eq!(local_copy.title, "Receipt-only Local");
    assert_eq!(local_copy.password, "receipt-only-password");
}

#[test]
fn restart_finishes_a_completed_conflict_copy_when_remote_is_temporarily_unavailable() {
    let (base_bytes, local_entry_id) = vault_with_entry_bytes();
    let (remote_bytes, remote_entry_id) = foreign_remote_head_bytes(&base_bytes);
    let mut harness = RuntimeProtocolHarness::resident_with_in_memory_vault(base_bytes);
    harness.command(RuntimeCommand::Handshake {
    ...
    harness.reject_next_publication_as_stale(remote_bytes);
    harness.interrupt_next_conflict_split_after_conflict_copy();
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateEntryFields {
            vault_id,
            entry_id: local_entry_id.clone(),
            title: "Completed Conflict Copy Local".into(),
            username: "base-user".into(),
            password: "completed-copy-password".into(),
            url: "https://base.example".into(),
            notes: "adoption still pending".into(),
            totp_uri: None,
            custom_fields: vec![],
        }),
        RuntimeResponse::Error(_)
    ...
    let active_entries = match harness.command(RuntimeCommand::ListEntries {
        ...
        RuntimeResponse::EntryList(entries) => entries.entries,
        other => panic!("expected recoverable Local entry list, got {other:?}"),
    };
    assert_eq!(active_entries.len(), 1);
    assert_eq!(active_entries[0].id, local_entry_id);
    ...
    match harness.command(RuntimeCommand::RetryVaultSourceSync {
        ...
        other => panic!("expected repeated outage to remain pending, got {other:?}"),
    }
    let still_local = match harness.command(RuntimeCommand::ListEntries {
        vault_id: restarted_vault_id.clone(),
    }) {
        RuntimeResponse::EntryList(entries) => entries.entries,
        other => panic!("expected Local entry list after repeated outage, got {other:?}"),
    };
    assert_eq!(still_local.len(), 1);
    assert_eq!(still_local[0].id, local_entry_id);
    ...
    let adopted_remote = match harness.command(RuntimeCommand::ListEntries {
        vault_id: restarted_vault_id,
    }) {
        RuntimeResponse::EntryList(entries) => entries.entries,
        other => panic!("expected active Remote entry list after recovery, got {other:?}"),
    };
    assert_eq!(adopted_remote.len(), 1);
    assert_eq!(adopted_remote[0].id, remote_entry_id);
    ...
    assert_eq!(
        conflict_items.len(),
        1,
        "recovery must reuse the completed Conflict Copy"
    );
}
...
fn harness_keeps_browser_commands_behind_the_protocol_session_boundary() {
    ...
    assert!(matches!(
        ...
        RuntimeResponse::Error(error) if error.code == "runtime_error"
    ));
}

#[test]
fn browser_login_mutations_commit_once_and_report_pending_publication() {
    let mut harness =
    ...
    let vault_id = match harness.command(RuntimeCommand::GetSessionState) {
        RuntimeResponse::SessionState(state) => {
            assert!(state.unlocked);
            state.active_vault_id.expect("active browser vault")
        }
        other => panic!("expected unlocked browser session, got {other:?}"),
    };
    let root_id = match harness.command(RuntimeCommand::GetAutofillCreateContext {
        ...
        other => panic!("expected browser create context, got {other:?}"),
    };
    ...
    let entry_id = match harness.command(RuntimeCommand::FindFillCandidates {
        ...
        RuntimeResponse::FillCandidates(candidates) => {
            assert_eq!(candidates.entries.len(), 1);
            candidates.entries.into_iter().next().unwrap().id
        }
        other => panic!("expected created browser candidate, got {other:?}"),
    };
    let expected_fields = match harness.command(RuntimeCommand::GetAutofillEntryFields {
        vault_id: vault_id.clone(),
        entry_id: entry_id.clone(),
        url: "https://example.com/login".into(),
    }) {
        RuntimeResponse::AutofillEntryFields(fields) => fields.fields,
        other => panic!("expected browser entry fields, got {other:?}"),
    };
    ...
    assert!(matches!(
        harness.command(RuntimeCommand::GetAutofillEntryFields {
            vault_id: vault_id.clone(),
            entry_id: entry_id.clone(),
            url: "https://example.com/login".into(),
        }),
        RuntimeResponse::AutofillEntryFields(fields)
            if fields.fields.password == "second-secret"
    ));
    assert!(matches!(
        harness.command(RuntimeCommand::UpdateAutofillEntryFields {
            vault_id,
            ...
            desired_fields: AutofillUpdateFieldsDto {
                ...
                url: "https://attacker.example/login".into(),
            },
        }),
