...
impl KeepassCore {
    ...
    pub fn add_entry_with_id(
        ...
        if find_group_by_id(&vault.root, parent_group_id).is_none() {
            return Err(MutationError::GroupNotFound(parent_group_id.into()));
        }
        let creation_time =
            next_creation_timestamp(group_subtree_deletion_at(vault, parent_group_id))?;
    ...
    pub fn add_group(
        ...
        new_group.times = Some(GroupTimes {
            created_at: creation_time,
            modified_at: creation_time,
            expires: false,
            expiry_time: Some(creation_time as i64),
            last_accessed_at: Some(creation_time),
            usage_count: Some(0),
            location_changed_at: Some(creation_time),
        });
        let parent = find_group_by_id_mut(&mut vault.root, parent_group_id)
            .expect("parent existence checked");
    ...
    pub fn move_entry(
        ...
        let target_group_uuid = find_group_by_id(&vault.root, target_group_id)
            .map(|group| group.id)
        ...
        let entry = find_entry_by_id(&vault.root, entry_id)
            .ok_or_else(|| MutationError::EntryNotFound(entry_id.into()))?;
        if previous_parent == target_group_uuid {
            return Ok(project_entry(entry));
        }
        let target_deletion = group_subtree_deletion_at(vault, target_group_id);
        let source_deletion = group_lineage_deletion_at(vault, previous_parent);
        let next_location =
            next_entry_location_timestamp(vault, entry, target_deletion, source_deletion)?;
    ...
    pub fn move_group(
        ...
        let target_parent_uuid = find_group_by_id(&vault.root, target_parent_group_id)
            .map(|group| group.id)
            .ok_or_else(|| MutationError::GroupNotFound(target_parent_group_id.into()))?;

        let previous_parent = find_group_parent_id(&vault.root, group_id)
            .ok_or_else(|| MutationError::GroupNotFound(group_id.into()))?;
        if previous_parent == target_parent_uuid {
            return Ok(project_group(moving_group));
        }
        let target_deletion = group_subtree_deletion_at(vault, target_parent_group_id);
        let source_deletion = group_lineage_deletion_at(vault, previous_parent);
        let next_location =
            next_group_location_timestamp(vault, moving_group, target_deletion, source_deletion)?;
        let (mut moved_group, removed_from) = take_group_from_group(&mut vault.root, group_id)
            .ok_or_else(|| MutationError::GroupNotFound(group_id.into()))?;
        debug_assert_eq!(removed_from, previous_parent);
    ...
    pub fn soft_delete_entry_to_recycle_bin(
        ...
        let previous_parent =
            find_entry_parent_id(&vault.root, entry_id).expect("entry existence checked");
        if vault.recycle_bin_group == Some(previous_parent) {
            return Ok(project_entry(entry));
        }
        let recycle_bin_deletion = match vault.recycle_bin_group {
            Some(recycle_bin_id)
                if find_group_by_id(&vault.root, &recycle_bin_id.to_string()).is_some() =>
            {
                group_subtree_deletion_at(vault, &recycle_bin_id.to_string())
            }
            _ => group_subtree_deletion_at(vault, &vault.root.id.to_string()),
        };
        let next_location = next_entry_location_timestamp(
            vault,
            entry,
            recycle_bin_deletion,
            group_lineage_deletion_at(vault, previous_parent),
        )?;
        let recycle_bin_id = ensure_recycle_bin(vault);
        let (mut entry, parent_group_id) = take_entry_from_group(&mut vault.root, entry_id)
            .ok_or_else(|| MutationError::EntryNotFound(entry_id.into()))?;
        debug_assert_eq!(parent_group_id, previous_parent);
    ...
    pub fn restore_entry_from_recycle_bin(
        ...
        let recycle_bin_uuid = vault
            .recycle_bin_group
            .ok_or_else(|| MutationError::GroupNotFound("recycle-bin".into()))?;
        let recycle_bin_id = recycle_bin_uuid.to_string();
        let (target_group_id, next_location) = {
            ...
            if target_group_id == recycle_bin_id {
                return Ok(project_entry(entry));
            }
            let target_deletion = group_subtree_deletion_at(vault, &target_group_id);
            (
                target_group_id,
                next_entry_location_timestamp(
                    vault,
                    entry,
                    target_deletion,
                    group_lineage_deletion_at(vault, recycle_bin_uuid),
                )?,
            )
        };
...
fn find_group_parent_id(group: &Group, id: &str) -> Option<Uuid> {
    if group
        .children
        .iter()
        .any(|child| child.id.to_string() == id)
    {
        return Some(group.id);
    }
    for child in &group.children {
        if let Some(found) = find_group_parent_id(child, id) {
            return Some(found);
        }
    }
    None
...
fn find_entry_parent_id(group: &Group, id: &str) -> Option<Uuid> {
    if group.entries.iter().any(|entry| entry.id.to_string() == id) {
        return Some(group.id);
    }
    for child in &group.children {
        if let Some(found) = find_entry_parent_id(child, id) {
            return Some(found);
        }
    }
    None
...
fn next_entry_location_timestamp(
    vault: &Vault,
    entry: &Entry,
    target_deletion: Option<i64>,
    source_deletion: Option<i64>,
) -> Result<u64, MutationError> {
    let tombstone_floor = [
        latest_tombstone_timestamp(vault, entry.id),
        source_deletion,
        target_deletion,
    ]
    .into_iter()
...
fn next_creation_timestamp(target_deletion: Option<i64>) -> Result<u64, MutationError> {
    let tombstone_floor = target_deletion.map(i128::from).unwrap_or(i128::MIN);
    let next = i128::from(current_unix_timestamp().max(0)).max(tombstone_floor.saturating_add(1));
    let next = u64::try_from(next).map_err(|_| {
        MutationError::InvalidEntryValue(format!("timestamp is outside the KDBX4 domain: {next}"))
    })?;
    validate_persistent_unsigned_timestamp(next)?;
...
fn next_group_location_timestamp(
    vault: &Vault,
    group: &Group,
    target_deletion: Option<i64>,
    source_deletion: Option<i64>,
    ...
    let tombstone_floor = [
        latest_tombstone_timestamp(vault, group.id),
        source_deletion,
        target_deletion,
    ]
    .into_iter()
...
fn latest_tombstone_timestamp(vault: &Vault, id: Uuid) -> Option<i64> {
    vault
        .deleted_objects
        .iter()
        .filter(|deleted| deleted.id == id)
...
fn group_lineage_deletion_at(vault: &Vault, group_id: Uuid) -> Option<i64> {
    fn collect_parents(
        group: &Group,
        current_parent: Option<Uuid>,
        parents_by_group: &mut BTreeMap<Uuid, BTreeSet<Uuid>>,
    ) {
        ...
        for child in &group.children {
            collect_parents(child, Some(group.id), parents_by_group);
        }
    }

    let mut parents_by_group = BTreeMap::<Uuid, BTreeSet<Uuid>>::new();
    collect_parents(&vault.root, None, &mut parents_by_group);
    ...
    while let Some(id) = pending.pop() {
        ...
        latest = [latest, latest_tombstone_timestamp(vault, id)]
            .into_iter()
            .flatten()
            .max();
        if let Some(parents) = parents_by_group.get(&id) {
...
fn group_subtree_deletion_at(vault: &Vault, target_group_id: &str) -> Option<i64> {
    fn visit(
        vault: &Vault,
        group: &Group,
        target_group_id: &str,
        ...
        let group_deletion = [
            ...
            group
                .previous_parent
                .and_then(|id| group_lineage_deletion_at(vault, id)),
        ]
        .into_iter()
        .flatten()
        .max();
        let location_changed_at = group
            .times
            .and_then(|times| times.location_changed_at)
            .unwrap_or(0);
        let subtree_deletion = group_deletion
            .filter(|deleted_at| i128::from(*deleted_at) > i128::from(location_changed_at));
        if group.id.to_string() == target_group_id {
            return Some(subtree_deletion);
        }
        for child in &group.children {
            if let Some(found) = visit(vault, child, target_group_id, subtree_deletion) {
                return Some(found);
            }
        }
        None
    ...
    visit(vault, &vault.root, target_group_id, None).unwrap_or(None)
}
...
mod internal_tests {
    ...
    #[test]
    fn add_entry_with_id_advances_creation_past_parent_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("create entry after ancestor deletion");
        let mut ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let target = Group::new("target");
        let target_id = target.id;
        ancestor.children.push(target);
        vault.root.children.push(ancestor);
        ...
        let entry = &vault.root.children[0].children[0].entries[0];
        assert!(i128::from(entry.created_at) > i128::from(deleted_at));
        assert!(i128::from(entry.modified_at) > i128::from(deleted_at));
        assert!(i128::from(entry.location_changed_at.expect("location")) > i128::from(deleted_at));
    ...
    #[test]
    fn add_entry_with_id_is_atomic_when_parent_tombstone_has_no_successor() {
        const MAX_KDBX4_TIMESTAMP: i64 = i64::MAX - 62_135_596_800;

        let core = KeepassCore::new();
        let mut vault = Vault::empty("entry creation clock overflow");
        let target = Group::new("target");
        let target_id = target.id;
        vault.root.children.push(target);
        vault.deleted_objects.push(DeletedObject {
            id: target_id,
            deleted_at: MAX_KDBX4_TIMESTAMP,
        });
        let before = vault.clone();
    ...
    #[test]
    fn add_group_advances_creation_past_parent_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("create group after ancestor deletion");
        let mut ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let target = Group::new("target");
        let target_id = target.id;
        ...
        let created = core
            .add_group(&mut vault, &target_id.to_string(), "created")
            .expect("create group");

        let group = &vault.root.children[0].children[0].children[0];
        assert_eq!(group.id.to_string(), created.id);
        let times = group.times.expect("created group times");
        assert!(i128::from(times.created_at) > i128::from(deleted_at));
        assert!(i128::from(times.modified_at) > i128::from(deleted_at));
        assert!(i128::from(times.location_changed_at.expect("location")) > i128::from(deleted_at));
        let mut tombstones = Vault::empty("tombstones");
        tombstones.root.id = vault.root.id;
        tombstones.deleted_objects = vault.deleted_objects.clone();
        vault.merge_from(&tombstones);
        assert!(core.find_group_view_by_id(&vault, &created.id).is_some());
    }

    #[test]
    fn add_group_is_atomic_when_parent_tombstone_has_no_successor() {
        const MAX_KDBX4_TIMESTAMP: i64 = i64::MAX - 62_135_596_800;

        let core = KeepassCore::new();
        let mut vault = Vault::empty("group creation clock overflow");
        let target = Group::new("target");
        let target_id = target.id;
        vault.root.children.push(target);
        vault.deleted_objects.push(DeletedObject {
        ...
        assert!(matches!(
            core.add_group(&mut vault, &target_id.to_string(), "created"),
            Err(MutationError::InvalidEntryValue(_))
        ));
        assert_eq!(vault, before);
    }
    ...
    #[test]
    fn move_entry_to_current_parent_is_a_noop_and_preserves_restore_lineage() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("same-parent entry move");
        let mut active = Group::new("active");
        ...
        core.soft_delete_entry_to_recycle_bin(&mut vault, &entry_id.to_string())
            .expect("move to recycle bin");
        let before_same_parent_move = vault.clone();
        ...
        assert_eq!(vault, before_same_parent_move);
        core.restore_entry_from_recycle_bin(&mut vault, &entry_id.to_string(), None)
            .expect("restore to original parent");
        assert_eq!(vault.root.children[0].id, active_id);
        assert_eq!(vault.root.children[0].entries[0].id, entry_id);
        assert!(vault.root.children[1].entries.is_empty());
    }

    #[test]
    fn move_group_to_current_parent_is_a_noop() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("same-parent group move");
        let old_parent = Group::new("old parent");
        let old_parent_id = old_parent.id;
        let mut current_parent = Group::new("current parent");
        let current_parent_id = current_parent.id;
        let mut moved = Group::new("moved");
        let moved_id = moved.id;
        moved.previous_parent = Some(old_parent_id);
        ...
        current_parent.children.push(moved);
        vault.root.children.extend([old_parent, current_parent]);
        let before_same_parent_move = vault.clone();
    ...
    #[test]
    fn repeated_soft_delete_is_a_noop_and_preserves_restore_lineage() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("repeated soft delete");
        let mut active = Group::new("active");
        let active_id = active.id;
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        active.entries.push(entry);
        vault.root.children.push(active);

        core.soft_delete_entry_to_recycle_bin(&mut vault, &entry_id.to_string())
            .expect("first soft delete");
        let before_second_delete = vault.clone();

        core.soft_delete_entry_to_recycle_bin(&mut vault, &entry_id.to_string())
            .expect("second soft delete");

        assert_eq!(vault, before_second_delete);
        core.restore_entry_from_recycle_bin(&mut vault, &entry_id.to_string(), None)
            .expect("restore to original parent");
        assert_eq!(vault.root.children[0].id, active_id);
        assert_eq!(vault.root.children[0].entries[0].id, entry_id);
    ...
    #[test]
    fn restore_to_recycle_bin_is_a_noop_and_preserves_default_restore_lineage() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("same-parent restore");
        let mut active = Group::new("active");
        let active_id = active.id;
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        ...
        core.soft_delete_entry_to_recycle_bin(&mut vault, &entry_id.to_string())
            .expect("move to recycle bin");
        let before_same_parent_restore = vault.clone();

        core.restore_entry_from_recycle_bin(
            &mut vault,
            &entry_id.to_string(),
            Some(&recycle_bin_id.to_string()),
        )
        .expect("same-parent restore");

        assert_eq!(vault, before_same_parent_restore);
        core.restore_entry_from_recycle_bin(&mut vault, &entry_id.to_string(), None)
            .expect("restore to original parent");
        assert_eq!(vault.root.children[0].id, active_id);
        assert_eq!(vault.root.children[0].entries[0].id, entry_id);
    ...
    #[test]
    fn move_entry_advances_location_past_source_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move after source ancestor deletion");
        let mut ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let mut source = Group::new("source");
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        source.entries.push(entry);
        ancestor.children.push(source);
        let destination = Group::new("destination");
        let destination_id = destination.id;
        vault.root.children.extend([ancestor, destination]);
        let deleted_at = super::current_unix_timestamp()
            .checked_add(10_000)
            .expect("future tombstone");
        vault.deleted_objects.push(DeletedObject {
        ...
        let moved = &vault.root.children[1].entries[0];
        assert!(i128::from(moved.location_changed_at.expect("location")) > i128::from(deleted_at));
        let mut tombstones = Vault::empty("tombstones");
        tombstones.root.id = vault.root.id;
        tombstones.deleted_objects = vault.deleted_objects.clone();
    ...
    #[test]
    fn move_entry_advances_location_past_transitive_source_lineage_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move after transitive source deletion");
        let ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let mut intermediate = Group::new("intermediate");
        let intermediate_id = intermediate.id;
        intermediate.previous_parent = Some(ancestor_id);
        intermediate.times = Some(vaultkern_model::GroupTimes {
            created_at: 0,
            modified_at: 10,
            expires: false,
            expiry_time: None,
        ...
        let mut source = Group::new("source");
        source.previous_parent = Some(intermediate_id);
        source.times = Some(vaultkern_model::GroupTimes {
            created_at: 0,
            modified_at: 10,
            expires: false,
            expiry_time: None,
        ...
        vault
            .root
            .children
            .extend([ancestor, intermediate, source, destination]);
        let deleted_at = super::current_unix_timestamp()
            .checked_add(10_000)
            .expect("future tombstone");
        vault.deleted_objects.push(DeletedObject {
        ...
        let moved = &vault.root.children[3].entries[0];
        assert!(i128::from(moved.location_changed_at.expect("location")) > i128::from(deleted_at));
    }

    #[test]
    fn move_group_advances_location_past_source_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move group after source ancestor deletion");
        let mut ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let mut source = Group::new("source");
        let mut moved = Group::new("moved");
        ...
        vault.merge_from(&tombstones);
        assert!(
            core.find_group_view_by_id(&vault, &moved_id.to_string())
                .is_some()
        );
    }

    #[test]
    fn soft_delete_advances_location_past_source_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("recycle after source ancestor deletion");
        let mut ancestor = Group::new("ancestor");
        let ancestor_id = ancestor.id;
        let mut source = Group::new("source");
        let entry = Entry::new("entry");
        ...
        core.soft_delete_entry_to_recycle_bin(&mut vault, &entry_id.to_string())
            .expect("move entry to recycle bin");

        let recycle_bin_id = vault.recycle_bin_group.expect("recycle bin");
        let recycled = super::find_group_by_id(&vault.root, &recycle_bin_id.to_string())
            .and_then(|group| group.entries.first())
            .expect("recycled entry");
        assert!(
            i128::from(recycled.location_changed_at.expect("location")) > i128::from(deleted_at)
        );
        let mut tombstones = Vault::empty("tombstones");
        tombstones.root.id = vault.root.id;
        tombstones.deleted_objects = vault.deleted_objects.clone();
    ...
    #[test]
    fn move_group_advances_location_past_previous_parent_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move group after parent deletion");
        let mut source = Group::new("source");
        let source_id = source.id;
        let mut moved = Group::new("moved");
        let moved_id = moved.id;
        ...
        let moved = core
            .find_group_view_by_id(&vault, &moved_id.to_string())
            .expect("move survives the previous-parent tombstone");
        assert_eq!(moved.entries.len(), 1);
    }

    #[test]
    fn soft_delete_advances_location_past_previous_parent_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("recycle after parent deletion");
        let mut source = Group::new("source");
        let source_id = source.id;
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        source.entries.push(entry);
        vault.root.children.push(source);
        let deleted_at = super::current_unix_timestamp()
            .checked_add(10_000)
            .expect("future tombstone");
        vault.deleted_objects.push(DeletedObject {
    ...
    #[test]
    fn move_entry_advances_location_past_destination_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move into deleted destination");
        let mut source = Group::new("source");
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        source.entries.push(entry);
    ...
    #[test]
    fn move_group_advances_location_past_destination_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("move group into deleted destination");
        let mut source = Group::new("source");
        let mut moved = Group::new("moved");
        let moved_id = moved.id;
        moved.entries.push(Entry::new("subtree entry"));
        ...
        let moved = core
            .find_group_view_by_id(&vault, &moved_id.to_string())
            .expect("move survives the destination tombstone");
        assert_eq!(moved.entries.len(), 1);
    }

    #[test]
    fn soft_delete_advances_location_past_recycle_bin_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("recycle into deleted destination");
        let mut source = Group::new("source");
        let entry = Entry::new("entry");
        let entry_id = entry.id;
        source.entries.push(entry);
        let recycle_bin = Group::new("recycle bin");
        let recycle_bin_id = recycle_bin.id;
        vault.root.children.extend([source, recycle_bin]);
        vault.recycle_bin_enabled = Some(true);
        vault.recycle_bin_group = Some(recycle_bin_id);
        let deleted_at = super::current_unix_timestamp()
            .checked_add(10_000)
        ...
        let recycled = &vault.root.children[1].entries[0];
        assert!(
            i128::from(recycled.location_changed_at.expect("location")) > i128::from(deleted_at)
        );
        let mut tombstones = Vault::empty("tombstones");
    ...
    #[test]
    fn restore_is_atomic_when_recycle_bin_tombstone_has_no_successor() {
        const MAX_KDBX4_TIMESTAMP: i64 = i64::MAX - 62_135_596_800;

        let core = KeepassCore::new();
        let mut vault = Vault::empty("restore from deleted recycle bin");
    ...
    #[test]
    fn restore_entry_advances_location_past_recycle_bin_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("restore after recycle-bin deletion");
        let active = Group::new("active");
        let active_id = active.id;
        let mut recycle_bin = Group::new("recycle bin");
        let recycle_bin_id = recycle_bin.id;
    ...
    #[test]
    fn restore_entry_advances_location_past_recycle_bin_ancestor_tombstone() {
        let core = KeepassCore::new();
        let mut vault = Vault::empty("restore after recycle-bin ancestor deletion");
        let active = Group::new("active");
        let active_id = active.id;
        let mut ancestor = Group::new("recycle ancestor");
        let ancestor_id = ancestor.id;
        let mut recycle_bin = Group::new("recycle bin");
        let recycle_bin_id = recycle_bin.id;
        let mut entry = Entry::new("entry");
        ...
        let entry_id = entry.id;
        recycle_bin.entries.push(entry);
        ancestor.children.push(recycle_bin);
        vault.root.children.extend([active, ancestor]);
        vault.recycle_bin_enabled = Some(true);
        vault.recycle_bin_group = Some(recycle_bin_id);
        let deleted_at = super::current_unix_timestamp()
            .checked_add(10_000)
