...
describe("password change detection fill flow", () => {
  ...
  it("fails closed when same-form login and signup physical scopes coexist", () => {
    document.body.innerHTML = `
      <form>
        <section>
          <h2>Sign in</h2>
  ...
  it("does not reuse the current password across an unannotated password change", () => {
    document.body.innerHTML = `
      <form>
        <h2>Change password</h2>
    ...
    expect((document.querySelector("#new-password") as HTMLInputElement).value).toBe("");
    expect((document.querySelector("#password-confirmation") as HTMLInputElement).value).toBe("");
  });

  it("fails closed for unfocused ambiguous password-change roles", () => {
    document.body.innerHTML = `
      <form>
    ...
    fillLoginForm({ username: "alice@example.com", password: "old-secret" });
