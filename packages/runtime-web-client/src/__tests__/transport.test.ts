...
describe("createNegotiatedRuntimeTransport", () => {
  it("coalesces concurrent first-use handshakes before forwarding commands", async () => {
    ...
    const first = transport.send({ version: 3, command: { type: "first" } });
    const second = transport.send({ version: 3, command: { type: "second" } });
  ...
  it("renegotiates after a completed connection generation is replaced", async () => {
    ...
    await expect(
      transport.send({ version: 3, command: { type: "first" } })
    ).resolves.toEqual({ type: "first" });
    generation = 2;
    await expect(
      transport.send({ version: 3, command: { type: "second" } })
  ...
  it("does not cache a failed negotiation", async () => {
    ...
    await expect(
      transport.send({ version: 3, command: { type: "get_session_state" } })
